...
export const PiImage: React.FC<PiImageProps> = ({ size, landscapeSize = 'h-12' }) => {
  ...
  return (
    <div className="w-full relative bg-black/95 flex justify-center items-center min-h-0 pb-2">
      <img 
        src="/images/pi-no-background.png" 
        alt="Raspberry Pi"
        className={`${actualSize} w-auto object-contain min-h-0 text-green-400`}
        style={{
          filter: 'grayscale(100%) brightness(0.85) sepia(60%) hue-rotate(90deg) saturate(300%) brightness(0.9)',
          opacity: 0.85,
        }}
      />
    </div>
